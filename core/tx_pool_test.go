...
func (bc *testBlockChain) SubscribeBlockProcessingEvent(ch chan<- bool) event.Subscription {
	return bc.processFeed.Subscribe(ch)
}
...
func setupTxPoolWithConfig(config *params.ChainConfig) (*TxPool, *ecdsa.PrivateKey) {
	...
	blockchain := &testBlockChain{10000000, statedb, new(event.Feed), new(event.Feed)}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	blockchain := &testChain{&testBlockChain{1000000000, statedb, new(event.Feed), new(event.Feed)}, address, &trigger}
...
func TestTransactionChainFork(t *testing.T) {
	...
	resetState := func() {
		...
		pool.chain = &testBlockChain{1000000, statedb, new(event.Feed), new(event.Feed)}
		<-pool.requestReset(nil, nil)
	}
	resetState()
...
func TestTransactionPostponing(t *testing.T) {
	...
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	blockchain := &testBlockChain{1000000, statedb, new(event.Feed), new(event.Feed)}
