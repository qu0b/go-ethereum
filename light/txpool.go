...
type TxPool struct {
	...
	eip3860  bool // Fork indicator whether we are in the eip2718 stage.
}
...
func (pool *TxPool) setNewHead(head *types.Header) {
	...
	pool.eip3860 = pool.config.IsShanghai(new(big.Int).SetUint64(head.Time))
}
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	gas, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil, true, pool.istanbul, pool.eip3860)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
